...
for j = 1:sx
    ...
    file_name = ['hf_3_model_output'];

    outfile1 = fopen([file_name, '.txt'],'w');

    for i=1:length(reshaped_output)
        if reshaped_output(i)>=0
         fprintf(outfile1, '%s\n',dec2hex(reshaped_output(i),11));
        else
         fprintf(outfile1, '%s\n',dec2hex(2^(11*4)+reshaped_output(i),11));
        end
    end
    fclose(outfile1);      
    ...
    for i=1:length(reshaped_output_full)
      for j=1:poly_fact
        if reshaped_output_full(i,j)>=0
         fprintf(outfile2, '%s ',dec2hex(reshaped_output_full(i,j),11));
        else
         fprintf(outfile2, '%s ',dec2hex(2^(11*4)+reshaped_output_full(i,j),11));
        end
      end
      fprintf(outfile2, '\n');    
    end                
